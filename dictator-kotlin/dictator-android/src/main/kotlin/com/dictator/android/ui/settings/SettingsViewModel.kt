    ...
    private fun loadSettings() {
        viewModelScope.launch {
            try {
                ...
                val dictatorServiceUrl = sharedPreferences.getString(
                ...
                val model = sharedPreferences.getString("provider_model", null)
                    ?: getDefaultModel(selectedProvider)
                val temperature = (sharedPreferences.getString("provider_temperature", "0.7") ?: "0.7")
                    .toDoubleOrNull() ?: 0.7
                val maxTokens = (sharedPreferences.getString("provider_max_tokens", "2048") ?: "2048")
                    .toIntOrNull() ?: 2048
        ...
        viewModelScope.launch {
            ...
            try {
                val provider = when (currentState.mode) {
                    SettingsMode.DictatorService -> AiProviderFactory.createProvider(
                        ...
                        ProviderConfig(
                            ...
                            baseUrl = currentState.dictatorServiceUrl.trim(),
                            model = "dictator-ai-default"
                        )
                    )
                    SettingsMode.DirectProvider -> AiProviderFactory.createProvider(
                        httpClient,
                        ProviderConfig(
                ...
                if (!provider.isConfigured()) {
                    throw IllegalStateException("Provider is not configured")
                }
                ...
                _state.value = _state.value.copy(
                    testConnectionStatus = "✓ Connection successful (${provider.getProviderType().name} / ${provider.getModelName()})"
                )
            } catch (e: Exception) {
                _state.value = _state.value.copy(
                    testConnectionStatus = "✗ Connection failed: ${e.message ?: "Unknown error"}"
                )
            }
        }
    }
            ...
            SettingsMode.DirectProvider -> {
                when (state.selectedProvider) {
                    ...
                    ModelProvider.OLLAMA -> {
                        if (state.baseUrl.isBlank()) errors.add("Ollama base URL is required")
                        else if (!isValidUrl(state.baseUrl)) errors.add("Invalid Ollama base URL format")
                    }
                    ModelProvider.OPENAI_COMPATIBLE -> {
                        if (state.baseUrl.isBlank()) errors.add("Base URL is required")
                        else if (!isValidUrl(state.baseUrl)) errors.add("Invalid base URL format")
                        if (state.apiKey.isBlank()) errors.add("API key is required")
                    }
                    ModelProvider.DICTATOR -> Unit
                }
